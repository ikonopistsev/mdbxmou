...
if (typeof global.gc !== "function") {
    throw new Error("finalizer-reference-cleanup.js requires --expose-gc");
}
...
function issueLostTransaction(env, dbi) {
    ...
    txn._debugFailNextDetach("deleteReference");
    return { txnRef: new WeakRef(txn), view };
}

(async () => {
    const dbPath = fs.mkdtempSync(
        path.join(os.tmpdir(), "mdbxmou-stage2-finalizer-cleanup-"),
    );
    ...
    await waitUntil(
        ...
        "transaction with a reference cleanup fault was not finalized",
    );
